 ...
 * resolves with the aggregated assistant text.
 *
  ...
  return new Promise((resolve, reject) => {
    ...
    const completedSub = client.onNotification("turn/completed", (params) => {
      ...
      if (c.turn.status !== "completed") {
        // Non-success terminal status. Codex may still have committed
        // work on disk before the failure surfaced - try a tight
        // rollout recovery before declaring failure.
      ...
      if (combined.length === 0) {
        // Empty completed turn. The task_complete + agent_message may
        // have landed on disk a beat before the RPC notification
        // reached us with stale items.
        recoverOrRejectViaRolloutPolling({
          deadlineMs: ROLLOUT_RECOVERY_FAST_WINDOW_MS,
          pollMs: ROLLOUT_RECOVERY_POLL_MS,
          getRolloutPath: resolveRolloutPath,
      ...
      // Success path. Drop the toast-suppress sentinel before the
      // envelope-level writeReply also writes it - consume-on-read
    ...
    client
      ...
      .catch((err) => {
        ...
        // started. Rollout-recovery here would deliver stale text.
        // The silent-completion watcher only resolves when
        // ourTurnObserved is true, so it cannot rescue with stale
        // text either.
        settle(() => reject(err));
      });
  });
