...
import {
  ...
  buildResetRow,
  type PickerRow,
} from "./openCodeSessionsRows";
import {
  fetchSessions,
  formatSessionDisplayName,
  pickInstanceForTarget,
  TARGET_CONFIGS,
} from "./openCodeSessionsSource";
import { showSwitchSessionPicker } from "./openCodeSwitchSessionPicker";
import { readOpenCodeRoutesConfigSnapshot } from "../../shared/bridge/openCodeRoutesConfigSnapshot";
 ...
 * (`WAT321_MCP_SERVER/bin/opencode/dispatch.mjs`) resolves a missing `session` arg to that
 * active value, so the menu's CURRENT row is the user-facing source
 * of truth and dispatch follows automatically.
 *
 * Rows live in `openCodeSessionsRows.ts`, data sources in
 * `openCodeSessionsSource.ts`, the switch sub-picker in
 * `openCodeSwitchSessionPicker.ts`.
 */
...
async function showSessionsPicker(
  ...
  const items: PickerRow[] = [
    ...
    { ...makeCancelItem(EPIC_HANDSHAKE_MENU_TEXT), rowKind: "cancel" },
  ];

  const pick = await vscode.window.showQuickPick<PickerRow>(items, {
    title: cfg.title,
