...
import { type DrainedReply, listLateReplies } from "../../engine/inbox/inboxReader";
import { EPIC_HANDSHAKE_MENU_TEXT, type Action, type DispatchAction } from "./menuActions";
...
export async function showLateRepliesPicker(
  ...
  while (replies.length > 0) {
    ...
    type RepliesPick =
      | (vscode.QuickPickItem & { rowKind: "reply"; reply: DrainedReply })
    ...
    const pick = await vscode.window.showQuickPick<RepliesPick>(items, {
