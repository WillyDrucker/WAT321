...
/**
 * Logger for the Epic Handshake tier: the interface the JSON-RPC
 * client, dispatcher, and installer log through, plus the VS Code
 * output-channel implementation behind it. Callers depend on the
 * interface only, so a test can inject a console-backed double.
 *
 ...
 * `src/WAT321_MCP_SERVER/bin/channelLog.mjs`.
 *
 * Raw `writeFileSync` is intentional here, not a miss against the
 * `engine/fs/atomicWrite.ts:writeFileAtomic` invariant: this is an
