 ...
 * content that happens to appear there never leaks into the output
 * channel. Returns the top-level key names only, truncated. */
function extractFrameShapeForLog(line: string): string {
  try {
...
export class AppServerClient {
  ...
  private dispatchFrame(line: string): void {
    ...
    // Cap at 200 chars and only log frame shape keys, not values,
    // to keep any accidental sensitive content out of the channel.
    const shape = extractFrameShapeForLog(line);
    this.logger.warn(
