...
export class SessionTokenWidget<TState extends { status: string }> implements vscode.Disposable {
  ...
  private animationsActive(): boolean {
    ...
    if (this.lastState?.status === "ok") {
      ...
      // Compact-completion flash keeps the ticker alive ONLY while
      // the flashUntil window is open. The instant
      // `Date.now() >= flashUntil` the ticker can self-suspend - the
      // next render falls through to the normal path even before the
  ...
  private render(): void {
    ...
    switch (state.status) {
      ...
      case "ok": {
        ...
        // with no observable pre-completion signal (Claude Code
        // buffers the `/compact` user entry until the operation
        // finishes; auto-compact never writes a pre-entry). The state
