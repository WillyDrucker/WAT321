...
## Key Rules
- **No user file modifications** - tools must be read-only. Never write to ~/.claude/, ~/.codex/, or any user config. The only file WAT321 writes is `~/.wat321/` (API cooldown timestamps for rate-limit protection).
