...
- **GPT-5.6 shows up in the Codex model picker, along with its two new reasoning levels.** Sol, Terra, and Luna all appear, each offering exactly the efforts it actually supports. That includes **Max** and **Ultra**, which arrived with the 5.6 family and which WAT321 was quietly dropping. Ultra shows on Sol and Terra and stays hidden on Luna, because Luna does not accept it. There is nothing to configure. Whichever models your Codex can run are the ones you see, and a level OpenAI adds later will appear without waiting on a WAT321 update.

### Changed

- **The model list now comes from the Codex that actually runs your work.** Every Codex on your machine shares one model file and overwrites it with whatever its own version knows, so if you have the ChatGPT extension installed next to the command-line tool, the two take turns clobbering each other. WAT321 was reading that file. A model your Codex fully supports could disappear from the picker, and a perfectly good session could be flagged as broken and offered for repair. WAT321 now asks your Codex directly which models it can run. When it cannot ask, it falls back to the old behavior and never blocks a session from resuming.

### Fixed

- **Your Claude session tooltip no longer reads "Caveat: The messages below were generated by..."** Sessions you opened with a slash command were showing that internal preamble as the title instead of the first thing you actually typed.
- **The reasoning-effort picker no longer hides Max and Ultra.** If you had never set a per-workspace model override, the picker fell back to the four levels every model shares, so the two newest never appeared even when your model supported them.
- **Restart Codex Bridge now clears the cached model list.** Restarting after the bridge had gone idle could leave WAT321 describing a Codex it was no longer talking to.
